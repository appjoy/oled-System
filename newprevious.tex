...
module with standard 16-bit (5, 6, 5) RGB setting.
%
The simple linear model (denoted as LM) cannot model the OLED power on modern phones
which violates the superposition property.
...
\label{subsec:LMLR}
Violation of superposition property has been observed
previously~\cite{mittal2012empowering,kim2013runtime}
and suggests the power draw of three subpixels
are not independent.
The simplest idea to accommodate dependence is to introduce liner regression,
...
as the intensity varies from 0 to 255. The derived models
...
and 13\%, 14\%, and 7\% for magenta on the 3 phones respectively.

A second reason such linear regression based models are inaccurate
...
of 4 colors, yellow, magenta and cyan, and white:
  {\small
  \begin{eqnarray}
  P&=&V\cdot [ 
...
\noindent
Note that the coefficients for the nonlinear terms are very small (close to zero)
...
colors well\footnote{We use 4-color monochromatic images to train these
  ...
  enough diversity.}, we can expect that they will have high prediction errors
...
Interestingly, the error is lower on Pixel 2, which shows
the least violation of superposition property in Figure~\ref{fig:initial_evaluation_2}.
