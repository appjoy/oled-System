...
manufactures of various phone components. For screen, the model just
...
% app-oblivious and content-oblivious
...
and power caculation (\S\ref{subsec:appl}).
...
screen for 10 minutes.  As shown in \S\ref{subsec:appl}, at a pixel
sampling ratio of 100, the latency of per frame recording is
4ms, 3ms, and 2ms on the three phones while incurring less than
0.2\% additional prediction error.  We then isolated the modified Battery Stats
process and measured its CPU usage; the average CPU utilization is
only 1.1\%, 0.9\%, and 0.6\% for Pixel 2, Moto Z3, and Pixel 4 respectively.
